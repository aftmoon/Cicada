...
//为一个uv应用旋涡效果
float2 ApplyVortex(float2 uv, float2 center, half intensity, half angle)
{
	half dis = distance(uv, center);
	half rot = lerp(intensity, 0, dis) + angle;
	uv = RotatePoint2(uv, center, radians(rot));
	return uv;
}

//为一个颜色应用变换效果
half4 ApplyChange(half4 mainColor, half4 changeColor, half4 shinyColor, half threshold, half progress)
{
    half shiny = saturate((progress - threshold) * 10);
    half change = If(step(1, progress), 1, shiny);
    half4 mixColor = lerp(shinyColor, changeColor, change);
    half4 color = If(step(threshold, progress), mixColor, mainColor);
    return color;
}
